...
------------------------------------VISTAS----------------------------------------------
create view UbHos as (
select H.Nombre, U.Pais
from Hospital H, Ubicacion U
where H.idHospital = U.idUbicacion)

select * from UbHos

create view DocCons as (
select C.[Numero de consultorio], D.[Nombre Doctor]
from Consultorio C, Doctor D
where C.idConsultorio = D.idDoctor)

select * from DocCons

create view DocHosp as (
select D.[Nombre Doctor], H.[Numero Hospital]
from Doctor D, Hospital H
where D.idDoctor = H.idHospital)

select * from DocHosp

create view PacCons as (
select P.Nombre, C.[Numero de consultorio]
from Paciente P, Consultorio C
where P.idPaciente = C.idConsultorio)

select * from PacCons

create view PacDoct as (
select P.Nombre, D.[Nombre Doctor]
from Paciente P, Doctor D
where P.idPaciente = D.idDoctor)

select * from PacDoct

create view PacHosp as (
select P.[Numero de paciente], H.[Numero Hospital]
from Paciente P, Hospital H
where P.idPaciente = H.idHospital)

select * from PacHosp

create view PacMedu as (
select D.[Numero de paciente], C.[Medula de Doctor]
from Doctor D, Consultorio C
where D.idDoctor = C.idConsultorio)

select * from PacHosp	 

create view AreaHos as (
select D.[Numero Hospital], D.Area
from Doctor D, Paciente P
where D.idDoctor = P.idPaciente)

select * from AreaHos

create view PacItal as (
select P.Problema, H.Nombre
from Paciente P, Hospital H
where P.idPaciente = H.idHospital)

select * from PacItal

create view EsCons as (
select U.Estado, C.[Numero de consultorio]
from Ubicacion U, Consultorio C
where U.idUbicacion = C.idConsultorio)
